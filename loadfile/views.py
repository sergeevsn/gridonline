        ...
        if handle_uploaded_file(request.FILES.get('input_file', None), request.FILES.get('input_file', None).name):
            ...
            if check_data_file(request):
                ...
                if f.cleaned_data['file_format'] == 'table':
                    ...
                    print(get_full_filename(request.session['input_file_name']))
                ...
                first_point_x, first_point_y = get_xy_from_inline_xline(first_point_inline, first_point_xline,
                                                                        regr_params.get('x_coefs'),
                                                                        regr_params.get('y_coefs'))
                second_point_inline = min(inlines)
                second_point_xline = max(xlines)
                second_point_x, second_point_y = get_xy_from_inline_xline(second_point_inline, second_point_xline,
                                                                          regr_params.get('x_coefs'),
                                                                          regr_params.get('y_coefs'))
                third_point_inline = max(inlines)
                third_point_xline = max(xlines)
                third_point_x, third_point_y = get_xy_from_inline_xline(third_point_inline, third_point_xline,
                                                                        regr_params.get('x_coefs'),
                                                                        regr_params.get('y_coefs'))

                return render(request, 'upload.html',
                              {'inline_min_max': f"{min(inlines)}-{max(inlines)}({int(inl_step)})",
...
def calc_xy(request):
    if request.method == 'POST':
        ...
        x, y = get_xy_from_inline_xline(int(request.POST.get('inline')), int(request.POST.get('xline')),
                                        regr_params.get('x_coefs'), regr_params.get('y_coefs'))
...
def calc_inline_xline(request):
    if request.method == 'POST':
        ...
        print(request.POST.get('x')[0], request.POST.get('y')[0])
        inline, xline = get_inline_xline_from_x_y(float(request.POST.get('x')), float(request.POST.get('y')),
                                                  regr_params.get('inline_coefs'), regr_params.get('xline_coefs'))
        print(inline, xline)
        return HttpResponse(f"INLINE = <b>{int(np.round(inline))}</b>, XLINE = <b>{int(np.round(xline))}</b>")
